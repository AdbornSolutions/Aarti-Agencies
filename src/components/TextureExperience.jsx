...
function TextureExperience({ stacked = false }) {
  ...
  if (stacked) {
    return <section ref={ref} className="w-full bg-night py-28 md:py-36" aria-label="Laminate textures"><div className="mx-auto max-w-[1600px] px-6 md:px-10"><h2 className="max-w-xl font-display text-[10vw] font-light leading-[0.95] text-ivory sm:text-[6vw] lg:text-[3.4vw]">
            Feel the Difference in Every Detail.
          </h2><div className="mt-14 space-y-16">{textures.map(
      (texture) => <article key={texture.code} className="grid gap-6 md:grid-cols-12 md:items-end"><div className="overflow-hidden md:col-span-8"><img
        src={texture.image}
        alt={`${texture.name} laminate finish`}
        loading="lazy"
        className="aspect-[16/9] w-full object-cover"
      /></div><div className="md:col-span-4"><p className="font-sans text-[11px] uppercase tracking-[0.3em] text-rust">{texture.name}</p><p className="mt-3 font-display text-2xl font-light leading-snug text-ivory">{texture.caption}</p><p className="mt-4 font-sans text-[10px] uppercase tracking-[0.24em] text-ivory/40">{texture.code}</p></div></article>
    )}</div><a
      href="#collections"
      {...hoverProps()}
  ...
  ><div className="mx-auto flex w-full max-w-[1600px] items-end justify-between px-6 pt-28 md:px-10 md:pt-32"><h2
  ...
  >
    ...
    style={{ color: current.ink }}
  >{String(index + 1).padStart(2, "0")} / {String(count).padStart(2, "0")}</span></div><div className="relative mt-10 flex-1 overflow-hidden"><motion.div className="flex h-full" style={{ x, width: `${count * 100}%` }}>{textures.map(
    (texture, i) => <div
      key={texture.name}
    ...
    /><span
      ...
      aria-hidden="true"
    >{texture.code}</span></motion.div></div>
  )}</motion.div></div><div className="mx-auto w-full max-w-[1600px] px-6 pb-12 md:px-10 md:pb-16"><div
  ...
  >{current.name}</p><p
    ...
    style={{ color: current.ink }}
  >{current.caption}</p></motion.div></div><a
    href="#collections"
    {...hoverProps()}
  ...
  >
    ...
    className="h-4 w-4 transition-transform duration-500 ease-editorial group-hover:translate-x-1.5"
    strokeWidth={1.3}
  /></a></div><div className="mt-6 flex gap-2" aria-hidden="true">{textures.map(
    (texture, i) => <motion.span
      key={texture.code}
