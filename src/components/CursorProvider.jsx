...
function CursorProvider({ enabled, children }) {
  ...
  useEffect(() => {
    ...
    finePointer.current = window.matchMedia("(pointer: fine)").matches && !window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  ...
  return <CursorContext.Provider value={api}>{children}{enabled && <motion.div
    aria-hidden="true"
  ...
  ><motion.span
    ...
    animate={{
      width: size,
      height: size,
      backgroundColor: state.label ? "rgba(182, 94, 60, 0.92)" : state.active ? "rgba(245, 241, 232, 0.12)" : "rgba(245, 241, 232, 0.95)",
      borderColor: state.label ? "rgba(182,94,60,0)" : "rgba(245,241,232,0.55)"
    }}
    transition={{ duration: 0.28, ease: [0.23, 1, 0.32, 1] }}
    style={{ mixBlendMode: state.label ? "normal" : "difference" }}
  ><AnimatePresence>{state.label && <motion.span
    key={state.label}
  ...
  >{state.label}</motion.span>}</AnimatePresence></motion.span></motion.div>}</CursorContext.Provider>;
}
export {
  CursorProvider,
  useCursor
