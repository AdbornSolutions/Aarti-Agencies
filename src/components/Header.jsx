...
function Header({ ready }) {
  ...
  return <motion.header
    ...
    className={`fixed inset-x-0 top-0 z-[100] transition-colors duration-500 ease-editorial ${scrolled || menuOpen ? "border-b border-ivory/10 bg-ink/85 backdrop-blur-xl" : "border-b border-transparent bg-transparent"}`}
  ><div className="mx-auto flex max-w-[1600px] items-center justify-between px-6 py-5 md:px-10"><a
    href="#top"
    {...hoverProps()}
  ...
  /></a><nav className="hidden items-center gap-8 lg:flex" aria-label="Primary">{navLinks.map(
    (link) => <a
      key={link}
      href="#top"
      {...hoverProps()}
    ...
    >{link}<span className="absolute -bottom-1.5 left-0 h-px w-full origin-right scale-x-0 bg-rust transition-transform duration-500 ease-editorial group-hover:origin-left group-hover:scale-x-100" /></a>
  ...
  ><SearchIcon className="h-4 w-4" strokeWidth={1.4} /></button><a
    href="#collections"
    {...hoverProps()}
  ...
  >{menuOpen ? <XIcon className="h-4 w-4" strokeWidth={1.4} /> : <MenuIcon className="h-4 w-4" strokeWidth={1.4} />}</button></div></div><motion.div
    initial={false}
    animate={{ height: menuOpen ? "auto" : 0, opacity: menuOpen ? 1 : 0 }}
    transition={{ duration: 0.28, ease: EASE }}
    className="overflow-hidden lg:hidden"
  ><nav className="flex flex-col gap-1 px-6 pb-8 pt-2" aria-label="Mobile">{navLinks.map(
    (link) => <a
      key={link}
      href="#top"
