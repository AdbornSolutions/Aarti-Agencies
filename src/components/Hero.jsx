...
const HEADLINE = ['Premium Laminates.', 'Reliable Distribution.', 'Across Vidarbha.'];
function Hero({ ready }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-end justify-between gap-10">
          <div className="max-w-5xl">
            ...
            >
              <span className="h-px w-14 bg-rust" />
              <span className="font-sans text-[10px] uppercase tracking-[0.3em] text-ivory/65">
                Arti Agencies
              </span>
            </motion.div>
            <h1 className="font-display text-[13vw] font-light leading-[0.86] text-ivory sm:text-[11vw] lg:text-[8.4vw]">
              {HEADLINE.map((line, i) => (
                <span key={line} className="block overflow-hidden">
                  ...
                  >
                    {i === 2 ? <em className="not-italic text-rust">{line}</em> : line}
                  </motion.span>
                </span>
              ))}
            </h1>
          ...
          >
            Nagpur — Serving trade partners across Vidarbha
          </motion.span>
        </div>
        <motion.div
        ...
        >
          <p className="max-w-md font-sans text-sm font-light leading-relaxed text-ivory/70">
            Nagpur-based wholesale laminate supplier and distributor serving the evolving needs of
            Vidarbha’s furniture, interior and building-material industry.
          </p>
      ...
      >
        <span className="font-sans text-[9px] uppercase tracking-[0.3em] text-ivory/45">
          Scroll
        </span>
        ...
        <ArrowDownIcon className="h-3.5 w-3.5 text-ivory/45" strokeWidth={1.2} />
      </motion.div>
    </section>
  );
}
