...
function WhyChoose() {
  ...
  /><div className="mt-20">{strengths.map(
    (item, i) => <div key={item.index}><DrawLine className="h-px w-full bg-ivory/15" delay={i * 0.05} /><motion.div
      initial={{ opacity: 0 }}
    ...
    >{item.index}</motion.span><h3 className="font-display text-3xl font-light text-ivory md:col-span-4 md:text-[2.2vw]">{item.title}</h3><p className="max-w-2xl font-sans text-sm font-light leading-loose text-ivory/55 md:col-span-6">{item.description}</p></motion.div></div>
  )}<DrawLine className="h-px w-full bg-ivory/15" /></div></div></section>;
}
export {
  WhyChoose
};
