...
function Categories() {
  ...
  ><div className="mx-auto max-w-[1600px] px-6 md:px-10"><div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between"><WordReveal
    as="h2"
  ...
  /><FadeUp delay={0.2}><p className="max-w-xs font-sans text-sm font-light leading-relaxed text-ivory/55">
              Five material families, each developed for a different kind of space and a different
              kind of light.
            </p></FadeUp></div><DrawLine className="mt-14 h-px w-full bg-ivory/15" /></div>{
  ...
  >{categories.map((category, i) => {
    ...
    /><div
      ...
      aria-hidden="true"
    /><div className="absolute inset-0 flex flex-col justify-between p-6"><span className="font-sans text-[10px] tracking-[0.28em] text-ivory/70">{category.index}</span><motion.div
      animate={{ y: active === i ? -8 : 0 }}
      transition={{ duration: 0.6, ease: EASE }}
    ><h3 className="font-display text-3xl font-light leading-tight text-ivory lg:text-[1.9vw]">{category.name}</h3><motion.p
      ...
      transition={{ duration: 0.5, ease: EASE }}
    >{category.blurb}</motion.p><span className="mt-6 flex h-10 w-10 items-center justify-center rounded-full border border-ivory/30 text-ivory transition-colors duration-300 ease-editorial group-hover:border-rust group-hover:bg-rust"><ArrowUpRightIcon
